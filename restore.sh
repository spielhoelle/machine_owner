...
# scp -rp $dumpfile $wh:/
# ssh $wh docker cp /$dumpfile machine_owner_db:/
# ssh $wh docker exec machine_owner_db /bin/bash -c "dumpfile=$(ls / | grep ".sql" | tail -n 1)"
# ssh $wh 'docker exec machine_owner_db /bin/bash -c "mysql -uwordpress -pwordpress machine_owner_db < /$(ls / | grep ".sql" | tail -n 1)"'
